...
def remove_twin_digits(box, unit, values):
	...
	if len(twin) != 0:
		for digit in values[box]:
			for u in unit:
				if (u not in twin) & (digit in values[u]):
...
"""
	Setting up units and peers for each box in grid.
	
	Includes diagonal units for implementing diagonal Sudoku constraint.
"""
rows = 'ABCDEFGHI'
cols = '123456789'
...
diag_units = [[s+t for s,t in zip(rows, cols)], [s+t for s,t in zip(rows[::-1], cols)]]
